...
<div class="leftpanel">
	<h3 class="accordion" id="section1">Expiries<span></span><h3>
		<div class="container">
		    <div class="content">
...
<h3 class="accordion" id="section5">View Reports<span></span><h3>
<div class="container">
    <div class="content">
    	 <h2>Click below to choose date range</h2>
	<input type="text" size="10"  value="" id="fromcommodity" placeholder="From" />
...
<h3 class="accordion" id="section6">Malaria Reports<span></span><h3>
<div class="container">
	    <div class="content">
	    	<h2>Click below to choose date range</h2>
			<input type="text" size="10"  value="" id="frommalariareport" placeholder="From" />
			<input type="text" size="10"  value="" id="tomalariareport" placeholder="To"/>
			<button class="awesome blue" id="malariareport" style="margin-left:30%">Generate Report</button>
	
	      	<!--<h3> <a href="#"  id="write_report">Create Report</a></h3>
	      	<h3> <a href="#"  id="view_report">View Reports</a></h3> -->
	    </div>
	</div>
</div>
...
$(function() {
	...
	$('#desc').change(function() {
        ...
        $("#commodityissue").click(function(){
   		 ...
   		 $("#malariareport").click(function(){
        	var url = "<?php echo base_url().'raw_data/malaria_reporthtml'?>";
		    var data_1 = "frommalariareport= "+$('#frommalariareport').val()+"&tomalariareport="+ $('#tomalariareport').val() +"&facilitycode="+ $('#facilitycode').val();
		    ajax_request (url,data_1);
   		 });
	    ...
	    $("#write_report").click(function(){
	    	var url = "<?php echo base_url().'report_Management/write_report'?>";
	    	ajax_request (url,'');
	    });
	    $("#view_report").click(function(){
	    	var url = "<?php echo base_url().'report_management/view_reports'?>";
	    	ajax_request (url,'');
	    });
		...
		$("#from,#from_order,#fromcommodity,#frommalariareport").datepicker({
			defaultDate : "+1w",
			changeMonth : true,
			changeYear : true,
			numberOfMonths : 1,
			onClose : function(selectedDate) {
				$("#to,#to_order,#tocommodity,#tomalariareport").datepicker("option", "minDate", selectedDate);
			}
		});
		
		$("#to,#to_order,#tocommodity,#tomalariareport").datepicker({
			defaultDate : "+1w",
			changeMonth : true,
			changeYear : true,
			numberOfMonths : 1,
			onClose : function(selectedDate) {
				$("#from,#from_order,#fromcommodity,#frommalariareport").datepicker("option", "maxDate", selectedDate);
			}
		});
